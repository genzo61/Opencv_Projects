...
def findmaxcounter(counters):
    ...
    for i in range(len(counters)):
        area_face = cv2.contourArea(counters[i])
...
while True:
    ...
    if len(counters) > 0:
        try:
            ...
            cv2.circle(roi,exbot,5,(0,255,0),2)

            cv2.line(roi,(exleft,extop),(0,0,255),2)
            cv2.line(roi,(extop,exright),(0,0,255),2)
            cv2.line(roi,(exright,exbot),(0,0,255),2)
            cv2.line(roi,(extop,exleft),(0,0,255),2)


            a = math.sqrt(exright[0]- extop[0] **2 + exright[1] - extop[1] ** 2)
            b = math.sqrt(exbot[0]- exright[0] **2 + exbot[1] - exright[1] ** 2)
            c = math.sqrt(exbot[0]- extop[0] **2 + exbot[1] - extop[1] ** 2)
            try:
